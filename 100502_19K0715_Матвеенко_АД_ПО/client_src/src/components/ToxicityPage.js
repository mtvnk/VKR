...
import { fetchToxicityData } from "../services/api";
...
const ToxicityPage = () => {
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true);
    setData(null);
    const response = await fetchToxicityData(inputText);
    setData(response);
    setLoading(false);
  };
  ...
  return (
    <div className="App">
      <h1>Анализ токсичности текста</h1>
